...
mod generated {
    wasmtime::component::bindgen!({
        ...
        imports: {
            ...
            // `websocket.protocol` and `websocket.close` are synchronous
            // functions in the WIT and are imported as such by guests, so
            // they are bound synchronously (still `trappable`, but not
            // `async`); they need no store access.
            ...
            // `receive-via-stream` and `state-changes` are synchronous in the
            // WIT: they hand back a stream without awaiting. They still need
            // `store` to allocate the returned stream on the guest's behalf.
            "lann:websocket/connections@0.1.0.[method]websocket.receive-via-stream": store | trappable,
            "lann:websocket/connections@0.1.0.[method]websocket.state-changes": store | trappable,
        },
        with: {
